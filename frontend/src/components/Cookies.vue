...
onMounted(() => {
  ...
  if (banner) {
    ...
    btnRejectAll?.addEventListener("click", () => {
      localStorage.setItem(
        "ucData",
        JSON.stringify({ ad_storage: "denied", analytics_storage: "denied" })
      );
      localStorage.setItem("ucDataExpire", new Date().getTime().toString());
      banner.classList.remove("show");
      messageApi.success("Preferences Updated!", 2);
...
<template>
  ...
  <div id="cookie-consent-banner" class="fixed bottom-0 left-0 w-full transition-all duration-300 p-2">
    <div
      class="rounded-lg text-black bg-gray-300/70 dark:text-white dark:bg-gray-800/70 border-t border-gray-200 dark:border-gray-600 px-10 py-6 backdrop-blur select-text transition-all duration-300">
      ...
      <div class="flex justify-between md:flex-row flex-col gap-4">
        ...
        <div
          ...
          <button id="btn-accept-all"
            class="cookie-consent-button btn-success font-[500] text-nowrap text-center px-3 py-1 border shadow-sm rounded-lg text-white hover:text-white bg-black dark:bg-[#1677ff] border-gray-700 shadow-transparent hover:border-gray-500 hover:shadow-gray-500 active:border-gray-700 active:shadow-gray-600 hover:opacity-75 disabled:bg-gray-600 disabled:text-gray-400 disabled:active:border-gray-400 disabled:hover:border-gray-400 disabled:shadow-transparent disabled:cursor-not-allowed transition-all duration-300 relative overflow-hidden">
            Accept All
          </button>
        </div>
      </div>
  ...
  <div id="cookie-settings-popup"
    class="cookie-settings-popup fixed inset-0 bg-gray-900 dark:text-white/95 bg-opacity-75 flex flex-col items-center justify-center z-50"
    style="display: none">
    ...
    <div class="flex w-full justify-stretch h-auto">
      ...
      <div
        class="flex-1 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg sm:min-w-[500px] sm:max-w-[500px] min-w-[90%] max-w-[90%] mx-2 z-10">
        <div class="flex justify-between mb-4">
          <h3 class="text-xl font-semibold">Privacy Settings</h3>
          <button id="closing-popup-element"
            class="text-lg font-semibold text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-100 translate-y-[-6px] transition duration-300">
            &times;
          </button>
        </div>
        <p class="text-sm">
          This tool helps you to select, activate or deactivate certain features
          on this website such as trackers, tags and analysis tools.
        </p>
        <div class="cookie-consent-options space-y-2 mt-4 overflow-auto max-h-[50vh]">
          <label
            class="flex items-center space-x-2 py-4 px-2 rounded-md border bg-gray-50 hover:bg-gray-50 border-gray-500 dark:bg-gray-700 dark:hover:bg-gray-700 dark:border-gray-600 relative">
            <div>
              <h4 class="mb-1 font-medium">Necessary</h4>
              <p class="text-sm italics text-gray-500 dark:text-gray-200 pr-20">
                Essential tags required to provide basic functionalities to the
                website.
              </p>
            </div>
          ...
          <label id="analytics-label"
            class="flex items-center space-x-2 py-4 px-2 rounded-md border bg-gray-50 hover:bg-gray-100 border-gray-500 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-500 relative cursor-pointer transition-all duration-300">
            <div>
              <h4 class="mb-1 font-medium">Analytics</h4>
              <p class="text-sm italics text-gray-500 dark:text-gray-200 pr-20">
                Analytical cookies used to understand visitor interactions with
                the website. They help us find areas of improvement to provide
          ...
          <label id="marketing-label"
            ...
            <div>
              ...
              <p class="text-sm italics text-gray-500 dark:text-gray-200 pr-20">
                Marketing cookies provide visitors with relevant advertisements
                based on their previous browsing activity. They also help us
